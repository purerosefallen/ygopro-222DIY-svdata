...
function c13254062.spfilter(c)
	return c:IsRace(RACE_FAIRY) and c:IsLevelBelow(1) and c:IsCanBeFusionMaterial(nil,true) and c:IsReleasable()
end
function c13254062.sprcon(e,c)
	if c==nil then return true end 
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-2
		and Duel.IsExistingMatchingCard(c13254062.spfilter,tp,LOCATION_MZONE,0,2,nil)
end
function c13254062.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.GetMatchingGroup(c13254062.spfilter,tp,LOCATION_MZONE,0,nil)
	local g1=nil
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RELEASE)
	g1=g:Select(tp,2,2,nil)
	c:SetMaterial(g1)
