...
function c80090022.initial_effect(c)
	...
	--xyz summon
	aux.AddXyzProcedure(c,aux.FilterBoolFunction(Card.IsSetCard,0x82d4),4,2)
	c:EnableReviveLimit()  
	--spsummon
...
function c80090022.filter(c,e,tp)
	return c:IsSetCard(0x82d4) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsCode(80090022)
end
...
function c80090022.filter1(c)
	return c:IsSetCard(0x82d4) and c:IsType(TYPE_MONSTER) and c:IsAbleToGrave()
end
function c80090022.filter2(c)
	return c:IsSetCard(0x82d4) and c:IsType(TYPE_SPELL+TYPE_TRAP) and c:IsAbleToGrave()
end
