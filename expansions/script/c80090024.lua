...
function c80090024.initial_effect(c)
	--synchro summon
	aux.AddSynchroProcedure(c,aux.FilterBoolFunction(Card.IsCode,80090000),aux.NonTuner(Card.IsSetCard,0x82d4),2)
	c:EnableReviveLimit()
	--spsummon
...
function c80090024.thfilter(c)
	return c:IsSetCard(0x82d4) and c:IsAbleToHand() and c:IsRace(RACE_FAIRY)
end
...
function c80090024.spfilter1(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and c:IsSetCard(0x82d4) and (not f or f(c))
