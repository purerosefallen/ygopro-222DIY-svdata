...
function c13254045.filter(c,e)
	return c:IsLevelBelow(1) and c:IsRace(RACE_FAIRY) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function c13254045.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then  
		e:SetLabel(0)
		return Duel.IsExistingTarget(Card.IsCanBeEffectTarget,tp,0,LOCATION_MZONE,1,nil)
	end
	local selA=0
	local selB=0
	local sel=0
	...
	if Duel.IsExistingMatchingCard(c13254045.filter,tp,LOCATION_DECK,0,1,nil,e) and Duel.IsExistingTarget(c13254045.filter2,tp,0,LOCATION_MZONE,1,nil) and Duel.GetLocationCount(1-tp,LOCATION_MZONE)>0 then
		selB=1
	end
	if selA==0 and selB==0 then return false end
...
function c13254045.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	elseif e:GetLabel()==1 and tc:IsRelateToEffect(e) and tc:IsLocation(LOCATION_MZONE) and tc:IsFaceup() then
		...
		local g=Duel.SelectMatchingCard(tp,c13254045.filter,tp,LOCATION_DECK,0,1,1,nil,e)
