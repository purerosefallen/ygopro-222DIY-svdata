...
function c8209736.fsfilter(c)
	return c:IsSetCard(0x3220) and c:IsType(TYPE_MONSTER) and c:IsType(TYPE_XYZ)
end
function c8209736.splimit(e,se,sp,st)
	return bit.band(st,SUMMON_TYPE_FUSION)==SUMMON_TYPE_FUSION
end
function c8209736.spfilter1(c,tp,fc)
	return c:IsSetCard(0x3220) and c:IsCanBeFusionMaterial(fc) and c:IsType(TYPE_XYZ) and Duel.CheckReleaseGroup(tp,c8209736.spfilter2,1,c,fc)
end
function c8209736.spfilter2(c,fc)
	return c:IsSetCard(0x3220) and c:IsCanBeFusionMaterial(fc) and c:IsType(TYPE_XYZ)
end
...
function c8209736.costfilter(c)
	return c:IsSetCard(0x3220) and c:IsAbleToRemoveAsCost()
end
