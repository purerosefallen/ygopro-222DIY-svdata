...
function c13254048.tgfilter1(c)
	return c:IsSetCard(0x1356) and c:IsType(TYPE_MONSTER) and c:IsAbleToGrave()
end
...
function c13254048.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local tc=g:GetFirst()
	if tc and Duel.SendtoGrave(tc,REASON_EFFECT)>0 and tc:IsLocation(LOCATION_GRAVE) and tc:IsSetCard(0x1356) and tc:IsType(TYPE_MONSTER)
		and Duel.IsExistingMatchingCard(c13254048.tgfilter1,tp,LOCATION_DECK,0,1,nil)
		and Duel.SelectYesNo(tp,aux.Stringid(13254048,0)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
		local g1=Duel.SelectMatchingCard(tp,c13254048.tgfilter1,tp,LOCATION_DECK,0,1,1,nil)
...
function c13254048.target2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c13254048.tgfilter2,tp,LOCATION_EXTRA,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,2,tp,LOCATION_EXTRA)
end
function c13254048.activate2(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c13254048.tgfilter2,tp,LOCATION_EXTRA,0,nil)
